describe('Cadastro e exclusão de equipamentos', ()=>{
    beforeEach(()=>{
        ...
        it('Deve logar, acessar a página de equipamentos, cadastrar um novo equipamento e excluí-lo', ()=>{
            ...
            cy.get('input[type=file]').first().selectFile('src/assets/css/img/tests/equipamento.jpeg')

            cy.wait(3000)

            cy.get('input[type=file]').last().selectFile('src/assets/css/img/tests/equipamento.jpeg')
            cy.get("input[type=text]").first().type('Equipamento teste')
